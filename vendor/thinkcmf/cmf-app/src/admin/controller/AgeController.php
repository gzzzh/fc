...
class AgeController extends AdminBaseController
{
    ...
    **/
    public function del()
    {
        ...
        //添加学科
         if(empty($id))
        {
            $this->error("不存在的年龄段，无法操作");
        }


        $info = Db::name('age_stage')->where('id', '=', $id)->find();
        if(empty($info)){
            $this->error("不存在的年龄段，无法修改");
        }

        //开始下架
        $status = ($info['status'] == 1) ? 2 : 1; 
        $res = Db::name('age_stage')->where('id', '=', $id)->update(['status' => $status]);
        if(empty($res)){
            $this->error("修改状态失败，系统错误");
        }

        $this->success('成功', 'age/index', '', 1);
    }
